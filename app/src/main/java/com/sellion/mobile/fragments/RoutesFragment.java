...
//    // Твой список дней (соответствует твоему XML и логике)
...
//
//        // --- ЛОГИКА ДЛЯ 2026 ГОДА (СЕГОДНЯ 10 ЯНВАРЯ - СУББОТА) ---
//        // Используем Calendar, чтобы точно определить текущий день
//        Calendar calendar = Calendar.getInstance();
//        int dayOfWeekNum = calendar.get(Calendar.DAY_OF_WEEK); // 7 для Субботы
//
//        int index;
//        switch (dayOfWeekNum) {
...
//            case Calendar.SATURDAY:  index = 5; break; // Здесь должно сработать
//            default: index = 0; // Воскресенье
//        }
//
//        String todayName = daysOfWeek[index];
//        tvCurrentDay.setText(todayName); // Это заменит "Понедельник" из XML на "Субботу"
//        loadStoresForDay(todayName);
//
//        // Обработка клика по кнопке "Назад" (из твоего XML)
...
//        // Определяем режим (мы внутри CreateOrder или CreateReturn)
//        boolean isReturnMode = false;
//        if (getParentFragment() instanceof CreateReturnFragment) {
//            isReturnMode = true;
//        }
//
//        List<DebtModel> filteredList = new ArrayList<>();
//        // Твой список магазинов (оставляю как пример, наполни своими данными)
//        switch (day) {
//            case "Суббота":
...
//                break;
//            // ... добавь остальные дни
//        }
//
//        // Используем DebtsAdapter для отображения списка магазинов в маршруте
//        DebtsAdapter adapter = new DebtsAdapter(filteredList, store -> {
//            // Вызываем метод onClientSelected у родительского фрагмента (CreateOrder или CreateReturn)
//            if (getParentFragment() instanceof CreateOrderFragment) {
//                ((CreateOrderFragment) getParentFragment()).onClientSelected(store.getShopName());
//            } else if (getParentFragment() instanceof CreateReturnFragment) {
//                ((CreateReturnFragment) getParentFragment()).onClientSelected(store.getShopName());
//            }
//        });
//
